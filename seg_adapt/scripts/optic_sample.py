...
from seg_adapt.guided_diffusion import dist_util, logger
from seg_adapt.guided_diffusion.script_util import (
    model_and_diffusion_defaults,
    create_model_and_diffusion,
    args_to_dict,
    add_dict_to_argparser,
)
from seg_adapt.guided_diffusion.image_datasets import load_data, ImageDataset
from OPTIC.networks.resnet import resnet18, resnet50
from OPTIC_SEG.networks.ResUnet_TTA_sample import ResUnet
from OPTIC_SEG.utils.convert import AdaBN
from torchvision import utils
import math
from torch.nn.parallel.distributed import DistributedDataParallel as DDP
from seg_adapt.optimization.image_editor_zecon import ImageEditor
...
def main():
    ...
    logger.log("creating the seg model")

    seg_model = ResUnet(resnet='resnet34', num_classes=2, pretrained=False, newBN=AdaBN, warm_n=5).to(dist_util.dev())
    checkpoint = torch.load(os.path.join('/lmx/data/OPTIC_CLASSIFY/OPTIC_SEG/models', args.source_dataset,'last-Res_Unet.pth'))
    ...
    for target_dataset in args.target_dataset:
        ...
        if target_dataset == 'REFUGE_Valid':
...
def create_argparser():
    defaults = dict(
        ...
        save_dir="/lmx/data/OPTIC_CLASSIFY/seg_generated",#saved data root path
        save_latents=False,
        out_ch=2,
        source_dataset='ACRIMA', # One of the dataset:   ['RIM_ONE_r3', 'REFUGE', 'Drishti_GS', 'ORIGA', 'ACRIMA']
        #target_dataset=['RIM_ONE_r3', 'ACRIMA'], 
        target_dataset=['RIM_ONE_r3', 'REFUGE', 'Drishti_GS', 'REFUGE_Valid', 'ORIGA']
        #target_dataset=['BKAI', 'CVC-ClinicDB', 'Kvasir-SEG', 'ETIS-LaribPolypDB']
    )
