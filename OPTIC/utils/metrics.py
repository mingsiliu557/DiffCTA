...
def calculate_accuracy(y_true, y_pred):
    ...
    correct_predictions = torch.sum(preds == y_true)
    accuracy = correct_predictions.float() / y_true.size(0)
    return accuracy

def calculate_cls_metrics(pred, label):
    acc = calculate_accuracy(label, pred)
    return [acc]
