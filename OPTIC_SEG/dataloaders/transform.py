...
def collate_fn_w_transform(batch):
    ...
    aug_dict = {'data': image, 'seg': None}
    aug_data = tr_transforms(**aug_dict)
    data_dict['data'] = aug_data['data']
    #data_dict['mask'] = to_one_hot_list(data_dict['mask'])
    return data_dict



def collate_fn_wo_transform_ensemble(batch):
    image, g_image, label, name = zip(*batch)
    image = np.stack(image, 0)
    g_image = np.stack(g_image, 0)
    label = np.stack(label, 0)
    name = np.stack(name, 0)
    data_dict = {'data': image, 'g_data': g_image, 'cls': label, 'name': name}
    #data_dict['mask'] = to_one_hot_list(data_dict['mask'])
    return data_dict
...
def collate_fn_wo_transform(batch):
    ...
    label = np.stack(label, 0)
    name = np.stack(name, 0)
    data_dict = {'data': image, 'g_data': g_image, 'mask': label, 'name': name}
    data_dict['mask'] = to_one_hot_list(data_dict['mask'])
    return data_dict
