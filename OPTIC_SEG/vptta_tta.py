...
from utils.tta import GTAug_A_2D, GTAug_B_2D
...
class VPTTA:
    ...
    def run(self):
        ...
        for batch, data in enumerate(self.target_test_loader):
            ...
            # Inference
            input_list = tta_test.img_list(x.cuda())  # 执行增强
            pred_list = []
            self.model.eval()
            # self.prompt.eval()
            with torch.no_grad():
                ...
                for x in input_list:
                    ...
                    pred_list.append(pred_logit)
                pred_logit_g, fea, _ = self.model(x_g)
                pred_list = tta_test.img_list_inverse(img_list=pred_list)
                
                pred_list_np = [pred_logit.cpu().numpy() for pred_logit in pred_list]
                pred_logit = np.mean(pred_list_np, axis=0)
                pred_logit = torch.from_numpy(pred_logit).to(self.device)
...
if __name__ == '__main__':
    ...
    config.Target_Dataset = ['RIM_ONE_r3', 'REFUGE', 'ORIGA', 'Drishti_GS']
