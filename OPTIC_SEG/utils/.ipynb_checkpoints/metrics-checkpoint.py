...
def crf_post_process(pred, image, num_classes):
    """
    使用 pydensecrf 进行后处理
    :param pred: 模型输出的概率图，形状为 (num_classes, height, width)，值范围 [0, 1]
    :param image: 原始 RGB 图像，形状为 (height, width, 3)，值范围 [0, 255]，类型为 uint8
    :param num_classes: 类别数
    :return: 优化后的概率分布，形状为 (num_classes, height, width)
    """
    # 确保概率图是 C-contiguous
    pred = np.ascontiguousarray(pred)

    # 确保图像是 C-contiguous 且类型为 uint8
    image = np.ascontiguousarray((image * 255).astype(np.uint8)) if image.dtype != np.uint8 else np.ascontiguousarray(image)
    ...
    # 将概率图转为对数概率（Unary Energy）
    unary = -np.log(pred + 1e-8)  # 防止 log(0)
    unary = unary.reshape((num_classes, -1))  # 形状为 (num_classes, height * width)
    unary = np.ascontiguousarray(unary)  # 确保 C-contiguous
    ...
    # 进行推理并获取优化后的概率分布
    Q = crf.inference(5)  # 进行 5 次推理
    Q = np.array(Q).reshape((num_classes, height, width))  # 确保返回形状为 (num_classes, height, width)

    return Q



def data_process(pred, label, threshold=0.5, abl_f = False, rsa_f = False):
    '''
...
# def data_process_crf(pred, label, image, threshold=0.5, abl_f = False, rsa_f = False):
#     '''
#     pred = np.array(pred*255).astype(np.uint8)
#     label = np.array(label*255).astype(np.uint8)
...
#     # 获取 batch_size, num_classes, height, width
#     b, c, h, w = pred.shape
...
#     for i in range(b):
#         # 当前图像的概率图和 RGB 图像
#         prob = pred[i]  # (num_classes, height, width)
#         img = image[i].transpose(1, 2, 0)  # 转换为 (height, width, 3)

#         # 确保概率图的范围在 [0, 1]
#         prob = np.clip(prob, 1e-8, 1.0)

#         # 使用 CRF 后处理
#         crf_result = crf_post_process(prob, img, num_classes=c)
#         optimized_preds.append(crf_result)

#     # 转换为 NumPy 数组
#     optimized_preds = np.array(optimized_preds)

#     # 阈值化处理
#     optimized_preds[optimized_preds >= threshold] = 1
#     optimized_preds[optimized_preds < threshold] = 0

#     # 继续其他后处理
#     for i in range(b):
#         for j in range(c):
#             optimized_preds[i, j] = post_process(abl_f, rsa_f, optimized_preds[i, j])

#     return optimized_preds.astype(np.uint8), label.astype(np.uint8)
...
def calculate_metrics(test, reference, image, rsa_f = False, abl_f = False):
    test, reference = data_process(pred=test, label=reference, threshold=0.5, abl_f=abl_f, rsa_f=rsa_f)
