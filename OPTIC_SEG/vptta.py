...
from utils.metrics import calculate_metrics, select_ensemble, dynamic_weighted_mixing
...
from utils.tta import TTA_2d as tta
...
class VPTTA:
    def __init__(self, config):
        ...
        target_test_dataset = OPTIC_dataset(config.dataset_root, config.generate_root,ts_img_list, ts_label_list,
    ...
    def run(self):
        ...
        for batch, data in enumerate(self.target_test_loader):
            x, x_g, y = data['data'], data['g_data'], data['mask']
            x = torch.from_numpy(x).to(dtype=torch.float32)
            x_g = torch.from_numpy(x_g).to(dtype = torch.float32)
            y = torch.from_numpy(y).to(dtype=torch.float32)

            x, x_g, y = Variable(x).to(self.device), Variable(x_g).to(self.device), Variable(y).to(self.device)
            ...
            with torch.no_grad():
                ...
                pred_logit, fea, _= self.model(x)
                pred_logit_g, fea, _ = self.model(x_g)

            # Update the Memory Bank
            #self.memory_bank.push(keys=low_freq.cpu().numpy(), logits=self.prompt.data_prompt.detach().cpu().numpy())

            # Calculate the metrics
            pred_logit = (pred_logit + pred_logit_g)/2
            #pred_logit = select_ensemble(pred_logit_g, pred_logit)
            #pred_logit = dynamic_weighted_mixing(pred_logit, pred_logit_g)
            
            seg_output = torch.sigmoid(pred_logit)
            metrics = calculate_metrics(seg_output.detach().cpu(), y.detach().cpu(), x.detach().cpu(), abl_f=True, rsa_f=False)
...
if __name__ == '__main__':
    ...
    # Path
    parser.add_argument('--path_save_log', type=str, default='/lmx/data/OPTIC_CLASSIFY/OPTIC_SEG/logs/')
    parser.add_argument('--model_root', type=str, default='/lmx/data/OPTIC_CLASSIFY/OPTIC_SEG/models/')
    parser.add_argument('--dataset_root', type=str, default='/lmx/data/OPTIC_CLASSIFY/Data')
    parser.add_argument('--generate_root', type=str, default='/lmx/data/OPTIC_CLASSIFY/seg_generated')
    ...
    config.Target_Dataset = ['RIM_ONE_r3', 'REFUGE', 'ORIGA', 'Drishti_GS', 'REFUGE_Valid']
    config.generate_root = os.path.join(config.generate_root, config.Source_Dataset + '_style')
